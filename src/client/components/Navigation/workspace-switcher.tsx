...
export function WorkspaceSwitcher() {
  ...
  const updateUserWorkspaceMutation = api.user.updateUserWorkspace.useMutation({
    onSuccess: (data) => {
      utils.invalidate();
      refetch();
  ...
  useEffect(() => {
    setCurrentWorkspaceId(user?.workspaceId ?? null);
  }, [user?.workspaceId]);
          ...
          >
            ...
            {workspaces?.map((workspace, index) => (
              <DropdownMenuItem
                key={workspace.name}
                onClick={() =>
                  updateUserWorkspaceMutation.mutate({
